...
terrenos, sendo que um deles deve ser necessariamente uma Ilha, como na figura
...
\textbf{virar} (em inglês, \textit{tap}): para jogar uma carta com custo,
é necessário virar o número de terrenos equivalente ao seu custo.
...
\begin{figure}[h!]
  \centering
  \caption{O estado $s_1 = \left\{ v_1 = v_2 = 10, h_1 = 4, h_2 = 0, B_2 = G_1 = G_2 = \emptyset, B_1, H \right\}.$ $B_1$ e $H$ são os seguintes:}
  \includegraphics[width=0.2\textwidth]{picstcc/bf1.png}
  \vskip1ex
  O campo de batalha $B_1$ consiste apenas em 4 terrenos (Montanhas).

  \vspace{.1cm}

  \includegraphics[width=0.7\textwidth]{picstcc/hand1.png}
  \vskip1ex
  A mão $H$ do agente consiste em um terreno (Montanha), um feitiço ``Volcanic Hammer'', um feitiço ``Flame Slash'' e uma criatura ``Pensive Minotaur''.
