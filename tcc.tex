...
Durante a Fase Principal é possível jogar cartas. Para que o conjunto de ações $A^P$ em uma Fase Principal seja não-vazio, é necessário que o agente seja o jogador ativo (o jogador ativo alterna todo turno). Definimos $A^P$ da seguinte maneira (cada ação depende de uma carta na mão e da condição especificada):

\begin{equation}
  A^P := \begin{lrdcases} Land, \textrm{\hfill se o jogador não jogou terreno neste turno} \\
                          Creature(c),\ \textrm{se o jogador possui $c$ terrenos desvirados na mesa} \\
                          Sorcery(c, T),\ \textrm{se o jogador possui $c$ terrenos desvirados na mesa e $T$ são alvos válidos} \\
                          CreatureWithEff(c, T),\ \textrm{se o jogador possui $c$ terrenos desvirados na mesa e $T$ são alvos válidos}, \\
                          Pass,\ \textrm{termina a fase.}
         \end{lrdcases}
\end{equation}
...
O conjunto $A^C_1$ de ações do jogador ativo no combate, portanto, é o conjunto da escolha de todas as combinações de criaturas atacantes (incluindo nenhuma). Dado que o jogador deve escolher alguma combinação, \begin{equation}|A^C_1| = \sum\limits_{i = 0}^{N}\binom{N}{i},\end{equation} onde $N$ é o número de criaturas que podem atacar na mesa do jogador ativo. 
