...
nível $i$ representa o número de cartas na mão inicial do jogador.}
\end{figure}
...
\frac{\binom{T}{j}\binom{60 - T}{i - j}}{\binom{60}{i}}, \ \
j = 0,\ldots, i, \end{equation} onde $t$ é o número de terrenos no deck
e $j$ o
número de terrenos
...
terrenos com probabilidade
 \[ \mathcal{P}_{i - 1}(j) = \frac{\binom{T}{j}\binom{60 - T}{i - 1 -
...
$(i, j), i \in \{ 0, 1, \ldots 7\}, j \in \{ 0, 1, \ldots, i \}$ (que representam os estados onde o jogador tem $i$ cartas na mão, sendo que $j$ são terrenos). Assim, as ações $A(s)$ e as probabilidades $P(s'|s, A(s))$ de cada estado $s$ são definidas da seguinte maneira (com $s \rightarrow s'$ representando a transição de estados):

\begin{itemize}
  \item $A(S_I) := \left\{ Start : S_I \rightarrow (7, j), j \in \{ 0, 1, \ldots, 7\} \right\} $;
  \\ \\
   A partir de $S_I$ é possível tomar a ação \textit{Start}, que leva para o estado $(7, j)$ com probabilidade \\ $P\left((7, j) |S_I, Start\right) = \mathcal{P}_7(j)$, descrita na equação \ref{eq:stateprob}.

  \item $A(i, j) := \begin{lrdcases} Mulligan : &(i, j) \rightarrow (i - 1, j'), \ \ i \in \{ 1, 2, \ldots, 7\}, j'\in \{ 0, 1, \ldots, i - 1\}, \\
                                     Keep :     &(i, j) \rightarrow S_F, \hspace{1.9cm} i \in \{ 0, 1, \ldots, 7\}, j\in \{ 0, 1, \ldots, i\} \end{lrdcases};$
   \\ \\
   A partir de $(i, j)$, analogamente à ação $Start$, temos a ação $Mulligan$, que leva ao estado $(i -1, j')$ com probabilidade $P((i-1, j') | (i, j), Mulligan) = \mathcal{P}_{i-1}(j')$. Um detalhe importante é a impossibilidade de $Mulligan$ quando $i = 0$. A outra ação, $Keep$, leva ao estado final com probabilidade $P(S_F | (i, j), Keep) = 1.$

   \pagebreak

   \item $A(S_F) := \left\{ Wait : S_F \rightarrow S_F \right\}$
   \\ \\ No estado final só há a ação $Wait$, um artifício para que o MDP sempre tenha o mesmo número de épocas de decisão (e constitua um problema de horizonte finito), com probabilidade $P(S_F | S_F, Wait) = 1$.
\end{itemize}
